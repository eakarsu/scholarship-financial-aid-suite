const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5160';
...
const adminCookie = await login('admin@scholarship-financial-aid.local', 'admin123');
const managerCookie = await login('manager@scholarship-financial-aid.local', 'manager123');
const analystCookie = await login('analyst@scholarship-financial-aid.local', 'analyst123');
